...
@section('content')
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">تیم ما /</span> لیست
    </h4>

    <!-- Responsive Table -->
    <div class="card">
        <div class="card-header" style="display: flex;flex-direction: row;justify-content: space-between">
            <h5>لیست افراد تیم</h5>
            <div class="align-items-center">
                <form action="{{route("teams.TeamsInfo")}}" method="get" id="searchFormSec">
                    @csrf
            ...
            <h6 class="btn btn-primary "><a class="text-white" href="{{route("teams.create")}}">ثبت آیتم جدید</a></h6>
        </div>
        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                ...
                @if(count($teams))
                    @foreach($teams as $team)
                        <tr>
                            ...
                            <td>{{$team->name}}</td>
                            <td>{{convertDateToFarsi($team->updated_at)}}</td>
                            <td>
                                <div class="dropdown">
                                    <button type="button" class="btn p-0 dropdown-toggle hide-arrow"
                                            data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button>
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{route('teams.edit',['id'=>$team->id])}}"><i
                                                class="bx bx-edit-alt me-2"></i> ویرایش</a>
                                        <a class="dropdown-item" href="{{route('teams.destroy',['id'=>$team->id])}}"><i
                ...
                @else
                    ...
                    @else
                        @php
                            $message="تاکنون عضو جدیدی برای تیم ثبت نشده است";
                        @endphp
                    @endif
                    <tr>
                        <th scope="row">{{$counter++}}</th>
